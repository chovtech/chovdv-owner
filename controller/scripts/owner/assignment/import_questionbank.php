<?php 
    ...
    $prosgetclass_current = mysqli_query($link, "SELECT * FROM `classordepartment` WHERE ClassOrDepartmentID='$class_id' AND CampusID='$campusid'");
    $prosgetclass_current_cnt = mysqli_num_rows($prosgetclass_current);
    $prosgetclass_current_cnt_row = mysqli_fetch_assoc($prosgetclass_current);



   $ClassTemplateID =  $prosgetclass_current_cnt_row['ClassTemplateID'];

   
   
      $select_question_category = $_POST["select_question_category"];

    if ($select_question_category === 'Objective') {

    $cat_new = "'Objective'";

    } elseif ($select_question_category === 'Theory') {

    $cat_new = "'Theory'";

    } else {

          $cat_new = "'Objective', 'Theory'";
    ...
    $session = "SELECT * FROM `session` WHERE `sessionStatus` = 1";
       ...
       $ekene_term = $ekene_get_term_details['TermOrSemesterID'];
         ...
         $ekene_import = "SELECT * FROM `questionbank`  WHERE `CampusID`= '$campusid' AND `ClassOrDepartmentID` = '$ClassTemplateID' AND `SubjectOrCourseID`= '$subject_id' AND 
        `sessionID`= '$ekene_session_name' AND `TermOrSemesterID`= '$ekene_term' AND  `QuestionCategory`= '$select_question_type' AND `QuestionType` IN ($cat_new)";
