...
$leave_id = $_POST['leave_id'];
    ...
    if($rows_for_staff_name > 0){
        ...
        if($rows_for_approve_or_reject > 0){
            ...
            $update_limit = "UPDATE `leaveapplication` SET `Status`= 2 WHERE `LeaveID`='$leave_id' AND `InstitutionID`='$keep_institution_id' AND `UserID` = '$keep_staff_id' ";
            $update_limit_query = mysqli_query($link, $update_limit);

            if($update_limit_query == true){
                echo 1;

                $select_for_approve_message = "SELECT * FROM `leaveapplication` WHERE `InstitutionID` = '$keep_institution_id' AND `LeaveID` = '$leave_id' AND `UserID` = '$keep_staff_id' AND `Status` = 2";
...
I hope this message finds you well. I regret to inform you that your leave application has been declined.

Leave Details:
- Leave Type: '.$get_leave_desc.'
- Dates: '.$get_leave_startdate.' to '.$get_leave_enddate.'

Please let me know if you have any questions or if theres anything else I can assist you with.
