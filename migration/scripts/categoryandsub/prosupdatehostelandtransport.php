  ...
  <?php
    ...
    'text/plain',
    'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'  // Add this line for .xlsx files
);
        ...
        if (!empty($_FILES['file']['name']) && in_array($_FILES['file']['type'], $csvMimes)) 
        {
            ...
            while (($line = fgetcsv($csvFile)) !== FALSE) 
            {
                      ...
                      function incrementHostName($name)
                        {
                            ...
                            if ($suffix !== '') {
                                ...
                                $new_suffix = $suffix + 1;
                                return preg_replace('/(\d+)$/', $new_suffix, $name);
                            } else {
                                // Append '1' as the numeric suffix
                                return $name . '1';
                            }
                        }
                ...
                //     //charge structure start here
                  ...
                  if($select_chargeold_cnt_rows > 0)
                  {
                                ...
                                do{
                                        ...
                                        // $select_transporthostel = mysqli_query($link,"SELECT * FROM `transportationtable` WHERE RouteName='$hosteltransportname' AND RouteAmount='$amount' AND `CampusID`='$abba_campus'");
                                        // $select_transportcntnew = mysqli_num_rows($select_transporthostel);
                                        
                                    
                                        // if($select_transportcntnew > 0)
                                        // {
                                        ...
                                        //         $insertroute = mysqli_query($link,"INSERT INTO `transportationtable`(`RouteID`, `InstitutionID`, `CampusID`, `RouteName`, `RouteAmount`, `DeleteStatus`) 
                                        //          VALUES ('$subcategoryIDOLD','$InstitutionIDgotten','$abba_campus','$hosteltransportname','$amount','0')");
                                            
                                        // }
                                       
                                       
                                        // $select_transportnew =  mysqli_query($link,"SELECT * FROM `hosteltable` WHERE  HostelName='$hosteltransportname' AND HostelAmount='$amount' AND CampusID='$abba_campus'");
                                        // $select_transportcntnew = mysqli_num_rows($select_transportnew);
                                        ...
                                        //      $inserttransport = mysqli_query($link,"INSERT INTO `hosteltable`(`HostelID`, `InstitutionID`, `CampusID`, `HostelName`, `HostelAmount`, `DeleteStatus`)
                                        //      VALUES ('$subcategoryIDOLD','$InstitutionIDgotten','$abba_campus','$hosteltransportname','$amount','0')");
                                        ...
                                        // Check if the hostel name and amount are the same
                                    $select_transporthostel = mysqli_query($link, "SELECT * FROM `transportationtable` WHERE RouteName='$hosteltransportname' AND RouteAmount='$amount' AND `CampusID`='$abba_campus'");
                                    $select_transportcntnew = mysqli_num_rows($select_transporthostel);
                                
                                    if ($select_transportcntnew > 0) {
                                        // Hostel name and amount already exist, use the same subcategory ID
                                        $select_transporthostel_row = mysqli_fetch_assoc($select_transporthostel);
                                        $subcategoryIDOLD = $select_transporthostel_row['RouteID'];
                                    } else {
                                        // Hostel name and amount do not exist, check if the name is the same
                                        $select_same_name = mysqli_query($link, "SELECT * FROM `transportationtable` WHERE RouteName='$hosteltransportname' AND `CampusID`='$abba_campus'");
                                        $select_same_name_cnt = mysqli_num_rows($select_same_name);
                                
                                        if ($select_same_name_cnt > 0) {
                                            // Increment the hostel name and use the same subcategory ID
                                            $hosteltransportname = incrementHostName($hosteltransportname);
                                        }
                                        ...
                                        AND CategoryID='$tranportID' AND SubcategoryID='$subcategoryIDOLD' AND Status='$status'");
                                        ...
                                        }else
                                        {
                                                ...
                                                VALUES ('$InstitutionIDgotten','$abba_campus','$tranportID','$subcategoryIDOLD','$sessionNamenew','$termOrSemesterNamegotennew','$ClassOrDepartmentID','$amount','$status')") ; 
                  ...
                  //transaction start here
                    ...
                    //               AND Session='$sessionNametransaction' AND Date='$dateCreated' AND StudentID='$studentIDtransaction' AND `CategoryID`='$tranportID' AND `SubcategoryID`='$subcategoryIDOLD'");
                    ...
                    //                             'income','Normal','$tranportID','$subcategoryIDOLD','Normal','$studentIDtransaction',
                ...
                //   //FEEWAVER HERE
                    ...
                    //                       AND Session='$sessionNamewave' AND Date='$sdateCreatedwave' AND StudentID='$studentIDwave' AND `CategoryID`='$tranportID' AND `SubcategoryID`='$subcategoryIDOLD'");
                    ...
                    //                                     'income','Normal','$tranportID','$subcategoryIDOLD','$wavenewstatype','$studentIDwave',
                ...
                //   //OPTIONAL FEE HERE 
                            ...
                            if($prosgetoptionalfeehere_cnt > 0)
                            {
                                                ...
                                                }else if($TermOrSemesteroptional == 'Third')
                                                {
                                                  ...
                                                  AND CategoryID='$tranportID' AND SubcategoryID='$subcategoryIDOLD' AND StudentID='$StudentIDoptional'");
                                                  ...
                                                  }else
                                                  {
                                                      ...
                                                      VALUES ('$InstitutionIDgotten','$abba_campus','$ClassOrDepartmentIDoptional','$SessionNameoptional', '$prosoptionalfeeterm', '$tranportID','$subcategoryIDOLD','$StudentIDoptional','$Status','$currentDate')");
